...
def getWorkingDir(): String = {
  ...
  val workingDir = "%s/%s/%s".format(getUserhome(), moduleName, langType)
...
def getDatabaseName(username:String, moduleName:String):String = {
  val user = username.replaceAll("[^a-zA-Z0-9]", "")
  val module = moduleName.replaceAll("[^a-zA-Z0-9]", "_")
  val langType = "scala" // for scal
  var databaseName = (module + "_" + user + "_" + langType).toLowerCase
  return databaseName
}

// Create a user-specific database
def createUserDatabase(username:String, moduleName:String):String = {
  val databaseName = getDatabaseName(username, moduleName)
...
def classroomCleanup(username:String, moduleName:String, dropDatabase: Boolean):Unit = {
  ...
  val database = getDatabaseName(username, moduleName)
  try {
...
val databaseName = createUserDatabase(username, moduleName)
