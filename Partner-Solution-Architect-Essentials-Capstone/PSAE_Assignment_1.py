...
# MAGIC # Partner Solution Architect Essentials - Assignment 1
# MAGIC 
# MAGIC ## Build a demo
# MAGIC 
# MAGIC __*Welcome!*__
...
df_backfill = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load(dataPath) 
#
# TO DO - Read the data from dataPath into a dataframe
# NOTES:
# - The input data is in CSV format
...
# MAGIC GROUP BY device_operational_status -- TO DO
# MAGIC ORDER BY device_operational_status -- TO DO
...
# MAGIC FROM sensor_readings_historical_bronze
# MAGIC WHERE 
# MAGIC   device_id = getArgument("PARAM_DEVICE_ID") -- TO DO use the getArgument function to grab values from the appropriate widget we defined above
# MAGIC   AND
# MAGIC   YEAR(reading_time) = getArgument("PARAM_YEAR") -- TO DO use the getArgument function to grab values from the appropriate widget we defined above
# MAGIC   AND
# MAGIC   MONTH(reading_time) = getArgument("PARAM_MONTH") -- TO DO use the getArgument function to grab values from the appropriate widget we defined above
# MAGIC   AND
# MAGIC   DAY(reading_time) = getArgument("PARAM_DAY") -- TO DO use the getArgument function to grab values from the appropriate widget we defined above
# MAGIC   AND 
# MAGIC   HOUR(reading_time) = getArgument("PARAM_HOUR") -- TO DO use the getArgument function to grab values from the appropriate widget we defined above
# MAGIC   AND
# MAGIC   MINUTE(reading_time) = getArgument("PARAM_MINUTE") -- TO DO use the getArgument function to grab values from the appropriate widget we defined above
# MAGIC   AND 
# MAGIC   SECOND(reading_time) BETWEEN getArgument("PARAM_START_SECOND") -- TO DO use the getArgument function to grab values from the appropriate widget we defined above 
# MAGIC     AND getArgument("PARAM_END_SECOND") -- TO DO use the getArgument function to grab values from the appropriate widget we defined above
...
# MAGIC USING historical_bronze_backfill_vw AS backfill -- TO DO... "AS" what?
# MAGIC ON silver.id = backfill.id
# MAGIC   -- TO DO... "ON" what?
...
# MAGIC MERGE INTO sensor_readings_historical_silver AS silver -- TO DO... "AS" what?
# MAGIC USING sensor_readings_historical_interpolations AS fix -- TO DO... "AS" what?
# MAGIC ON silver.id = fix.id
# MAGIC   -- TO DO... "ON" what?
# MAGIC WHEN MATCHED THEN UPDATE SET * -- TO DO... what?
...
# MAGIC 
# MAGIC SELECT * 
# MAGIC FROM sensor_readings_historical_silver VERSION AS OF 1 -- TO DO... how can you specify the time travel?
# MAGIC WHERE reading_1 = 999.99
...
display(dbutils.fs.ls(f"dbfs:/user/hive/warehouse/{database_name}.db/sensor_readings_historical_silver"))
...
display(dbutils.fs.ls(f"dbfs:/user/hive/warehouse/{database_name}.db/sensor_readings_historical_silver_by_device"))
...
display(dbutils.fs.ls(f"dbfs:/user/hive/warehouse/{database_name}.db/sensor_readings_historical_silver_by_hour/date_reading_time=2015-02-24/"))
...
# MAGIC %md
# MAGIC Imagine one or more Gold tables that Analysts and Data Scientists might want.  Create a few examples.

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Here is an example of a Gold table
# MAGIC CREATE OR REPLACE TEMP VIEW reading_1_avg AS (
# MAGIC   SELECT device_type, device_id, MEAN(reading_1) avg_1, date_trunc("DD", date_reading_time) date
# MAGIC   FROM sensor_readings_historical_silver_by_hour_and_minute
# MAGIC   GROUP BY device_type, device_id, date_trunc("DD", date_reading_time))
# MAGIC -- TO DO... create one or more Gold tables.  Focus on aggregations and filters that users might appreciate.

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM reading_1_avg