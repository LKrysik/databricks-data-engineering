...
# MAGIC # Partner Solution Architect Essentials - Assignment 2
# MAGIC 
...
# MAGIC 
# MAGIC -- <TO DO... you simply have to read from readilngs_streaming>
...
# MAGIC 
# MAGIC SELECT avg(reading_1) AS mean_reading_1
...
# MAGIC FROM readings_streaming 
# MAGIC GROUP BY 
# MAGIC   window(reading_time, '2 minutes', '1 minute'), -- NOTE that the windows uses a data column in the record.  It is NOT the current time in our code.
# MAGIC   device_type 
# MAGIC ORDER BY 
# MAGIC   window DESC, 
...
out_stream.writeStream \
          .format("delta") \
          .option("checkpointLocation", "/FileStore/flight/{dbfs_data_path}streaming_ckpnt") \
          .outputMode("complete") \
          .table("readings_agg")
...
# MAGIC 
# MAGIC select *, window.start AS window_start, window.end AS window_end FROM readings_agg
