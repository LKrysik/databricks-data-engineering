...
def writeToBronze(sourceDataframe, bronzePath, streamName):
  (sourceDataframe
    ...
    .filter(col("eventDate").isNull() != True)            
...
def loadStaticData(path):  
    return spark.read.option("inferSchema", "true").option("header", "true").load(path)
...
def bronzeToSilver(bronzePath, silverPath, streamName, lookupDF):
  (spark.readStream
    ...
    .select(col("eventParams.device_id").alias("device_id"),
            "eventName",
            col("eventParams.client_event_time").alias("client_event_time"),
            "eventDate"
           )
    .join(lookupDF, "device_id", "left_outer")
    .writeStream
    .format("delta")            
...
USING DELTA
LOCATION "dbfs:/user/txu@guidehouse.com/capstone/gaming/gold"
"""
).show()
...
display(
    spark.readStream.table("mobile_events_delta_gold"), streamName="bar_chart"
)
