...
display(airbnbDF.filter("room_type='Private room'"))
...
print(f"The formula for the linear regression line is y = {m}x + {b}")
...
pipeline = Pipeline(stages=[vecAssembler, lrModel])
model = pipeline.fit(trainDF)
predDF = model.transform(testDF)
display(predDF.select("bedrooms", "features", "price", "prediction"))

# COMMAND ----------

model.save("/ml/MyModel.model")

# COMMAND ----------

from pyspark.ml import PipelineModel
model = PipelineModel.load("/ml/MyModel.model")
