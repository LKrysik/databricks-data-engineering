...
products_ing = spark.read.format("xml").option("rootTag", "products").option("rowTag", "product").option("inferSchema", True).load(products_xml_path)
display(products_ing)

# COMMAND ----------

from pyspark.sql.functions import col
products_ing = products_ing.select(col("_product_id").cast("string").alias("product_id"),
                                   col("color"),
                                   col("model_name"),
                                   col("model_number"),
                                   ...
                                   col("price.usd").cast("double").alias("price"),
                                   col("size"))

# COMMAND ----------

products_ing1 = products_ing.where(col("price").isNotNull())
display(products_ing1)
