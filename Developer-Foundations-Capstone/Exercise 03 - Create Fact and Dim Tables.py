...
batch_source_path = spark.read.format("delta").load(batch_target_path)  

# COMMAND ----------

batch_source_path.createOrReplaceTempView("batch_temp_view")

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM batch_temp_view

# COMMAND ----------

spark.sql('DROP VIEW IF EXISTS batched_orders')
spark.sql('CREATE TEMP VIEW batched_orders AS SELECT * FROM batch_temp_view')
sqlContext.cacheTable("batched_orders")
...
batched_orders1 = batched_orders.withColumn("_error_ssn_format", when(col("sales_rep_ssn").contains("-"), "true").otherwise("false"))

# COMMAND ----------

from pyspark.sql.functions import regexp_replace
batched_orders1 = batched_orders1.withColumn('sales_rep_ssn', regexp_replace('sales_rep_ssn', '-', ''))

# COMMAND ----------

display(batched_orders1)

# COMMAND ----------

batched_orders2 = batched_orders1.select(col("sales_rep_id"),
                                         col("sales_rep_ssn").cast('long').alias("sales_rep_ssn"),
                                         col("sales_rep_first_name"),
                                         col("sales_rep_last_name"),
                                         col("sales_rep_address"),
                                         col("sales_rep_city"),
                                         col("sales_rep_state"),
                                         col("sales_rep_zip").cast("int").alias("sales_rep_zip"),
                                         col("ingest_file_name"),
                                         col("ingested_at"),
                                         col("_error_ssn_format").cast("boolean").alias("_error_ssn_format"))

# COMMAND ----------

batched_orders3 = batched_orders2.dropDuplicates(['sales_rep_id', 'sales_rep_ssn', 'sales_rep_first_name', 'sales_rep_last_name', 'sales_rep_address', 'sales_rep_city', 'sales_rep_state', 'sales_rep_zip', '_error_ssn_format'])

# COMMAND ----------

spark.sql('DROP TABLE IF EXISTS sales_rep_scd')
...
batched_orders4 = batched_orders.select(from_unixtime('submitted_at').alias('submitted_at'),
                                         col("order_id"),
                                         col("customer_id"),
                                         col("sales_rep_id"),
                                         ...
                                         col("shipping_address_zip").cast("int").alias("shipping_address_zip"),
                                         col("ingest_file_name"),
                                         col("ingested_at"))

# COMMAND ----------

batched_orders5 = batched_orders4.dropDuplicates(['submitted_at', 'order_id', 'customer_id', 'sales_rep_id', 'shipping_address_attention', 'shipping_address_address', 'shipping_address_city', 'shipping_address_state', 'shipping_address_zip'])
display(batched_orders5)

# COMMAND ----------

from pyspark.sql.functions import substring
batched_orders5 = batched_orders5.withColumn("submitted_yyyy_mm", substring('submitted_at', 1,7))

# COMMAND ----------

from pyspark.sql.functions import to_timestamp
batched_orders5 = batched_orders5.withColumn("submitted_at", to_timestamp(col("submitted_at")))
display(batched_orders5)
